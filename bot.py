...
def get_claim_url(user_data: dict, bc_uid: str, default_url: str = None) -> str:
    """Return claim URL with embedded user data as URL parameters."""
    if int((user_data or {}).get('marked_done') or 0):
        return DONE_BANNER_URL
    
    # Extract user data fields to embed in URL
    level = int((user_data or {}).get('level', 0))
    level_name = (user_data or {}).get('level_name', f'VIP {level}')
    username = (user_data or {}).get('username', 'User')
    wagered = float((user_data or {}).get('wagered', 0))
    wins = int((user_data or {}).get('wins', 0))
    bets = int((user_data or {}).get('bets', 0))
    medal_count = int((user_data or {}).get('medal_count', 0))
    
    # Build URL with all data embedded as parameters
